...
def get_segments(data, thresh, framerate):
    ...
    for l in data:
        # video name and sliding window length
        if "fg_name:" in l:
            vid = l.split('/')[-1] #video name

        # frame index, time, confident score
        elif "frames:" in l: #该输入的起始时间终止时间和步长
        ...
        elif "im_detect" in l: #处理结束
            return vid, segments #返回检测video_name 检测得到的action片段
...
def analysis_log(logfile, thresh, framerate):
    ...
    res = {}
    for l in lines:
        if "frames:" in l: #代表的一个输入样本检测结果的开始
            predict_data = []
        predict_data.append(l) #将接下来的结果添加进去
        if "im_detect:" in l: #一个检测结果的结束
...
parser.add_argument('--log_file', type=str, default='log_1.txt', help="test log file path")
parser.add_argument('--framerate', type=int, default=25, help="frame rate of videos extract by ffmpeg")
...
classes = generate_classes(META_DIR+'test', 'test', use_ambiguous=False) #返回一个字典 id-->classname
